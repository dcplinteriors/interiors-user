...
/// Full-screen multi-item submit form. Entered from a project (projectId locked)
/// or globally from "New request" (project picker).
class NewRequestView extends StatefulWidget {
  const NewRequestView({super.key, this.projectId});
...
class _NewRequestViewState extends State<NewRequestView> {
  ...
  ProjectsController get _projects => Get.find<ProjectsController>();
  MaterialRequestsController get _requests => Get.find<MaterialRequestsController>();

  bool get _locked => widget.projectId != null;

  /// True while any item still has an attachment uploading — submit waits for it.
  bool get _uploading => _items.any((d) => d.busy);

  /// True if any attachment failed to upload — submit is blocked until the user
  /// retries or removes it, so a failed attachment is never silently dropped.
  ...
  Project? _findProject(String? id) =>
      id == null ? null : _projects.projects.firstWhereOrNull((p) => p.id == id);
  ...
  Future<void> _close() async {
    ...
    final discard = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: Text(l10n.keepEditing)),
          FilledButton(
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) setState(() => _formError = 'Something went wrong. Please try again.');
    } finally {
            ...
            child: ListView(
              ...
              children: [
                ...
                for (var i = 0; i < _items.length; i++) ...[
                  _ItemCard(
                    // Identity key so per-row State (recorder, in-flight uploads)
                    // follows its draft when an earlier item is removed.
  ...
  Widget _projectSection(BuildContext context, AppLocalizations l10n) {
    final muted = Theme.of(context).colorScheme.onSurfaceVariant;
    if (_locked) {
      return Obx(() {
        final project = _findProject(widget.projectId);
        final placeholder = _projects.isLoading.value ? l10n.loadingProjects : '…';
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            InputDecorator(
              decoration: InputDecoration(labelText: l10n.projectLabel),
              child: Text(project?.particular ?? placeholder),
            ),
            if (project != null) ...[
              const SizedBox(height: 6),
              Text(l10n.poEcho(project.po), style: TextStyle(color: muted)),
            ],
          ],
        );
      });
    ...
    return Obx(() {
      ...
      // Distinguish loading / load-failed / genuinely-no-projects from a ready picker.
      if (projects.isEmpty) {
        final scheme = Theme.of(context).colorScheme;
        final (String text, Color color) = _projects.isLoading.value
            ? (l10n.loadingProjects, muted)
            : _projects.error.value != null
                ? (l10n.couldntLoadProjects, scheme.error)
                : (l10n.noProjectsAssigned, muted);
        return InputDecorator(
          decoration: InputDecoration(labelText: l10n.projectLabel),
          child: Text(text, style: TextStyle(color: color)),
        );
      }
      final selected = _findProject(_projectId);
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          DropdownButtonFormField<String>(
            ...
            items: [
              for (final p in projects)
                DropdownMenuItem(value: p.id, child: Text(p.particular)),
            ],
          ...
          if (selected != null) ...[
            const SizedBox(height: 6),
            Text(l10n.poEcho(selected.po), style: TextStyle(color: muted)),
          ],
        ],
      );
    });
...
/// The single audio note in progress. Keeps the bytes so a failed upload can be
...
class _ItemDraft {
  ...
  bool get busy => photos.any((p) => p.uploading) || (audio?.uploading ?? false);

  /// True if any attachment on this item failed to upload.
  bool get hasFailedUpload => photos.any((p) => p.failed) || (audio?.failed ?? false);

  /// The successfully-uploaded object paths (failed/in-flight uploads are excluded).
  Attachments get attachments => Attachments(
        photos: [for (final p in photos) if (p.path != null) p.path!],
        audio: audio?.path,
      );
...
class _ItemCard extends StatefulWidget {
  ...
  /// Asks the parent to rebuild so submit-enabled state re-evaluates as uploads
  /// start and finish.
...
class _ItemCardState extends State<_ItemCard> {
  ...
  Future<void> _addPhoto(ImageSource source) async {
    ...
    try {
      ...
      up = _PhotoUpload(
        bytes: bytes,
        contentType: photoContentType(bytes, mimeType: file.mimeType, fileName: file.name),
      );
    } catch (_) {
  ...
  Future<void> _startAudioUpload(String url) async {
    ...
    final up = _AudioUpload(bytes: recording.bytes, contentType: recording.contentType);
    _draft.audio = up;
    widget.onChanged();
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          runSpacing: 8,
          children: [
            for (final p in _draft.photos) _PhotoTile(upload: p, onRemove: () => _removePhoto(p), onRetry: () => _runPhotoUpload(p)),
            if (_draft.photos.length < _maxPhotos) _addPhotoTile(context, l10n),
          ],
        ),
        const SizedBox(height: 12),
        _audioControl(context, l10n),
      ],
    );
  }

  Widget _addPhotoTile(BuildContext context, AppLocalizations l10n) {
    final scheme = Theme.of(context).colorScheme;
  ...
  Widget _audioControl(BuildContext context, AppLocalizations l10n) {
    final scheme = Theme.of(context).colorScheme;
    if (_recording) {
    ...
    return _AudioChip(upload: audio, onRemove: _removeAudio, onRetry: () => _runAudioUpload(audio));
  }
...
/// An 80×80 photo thumbnail with an upload-state overlay (spinner / tap-to-retry)
/// and a remove button.
class _PhotoTile extends StatelessWidget {
  const _PhotoTile({required this.upload, required this.onRemove, required this.onRetry});
...
class _AudioChip extends StatelessWidget {
  const _AudioChip({required this.upload, required this.onRemove, required this.onRetry});
