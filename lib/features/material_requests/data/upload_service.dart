...
class ApiUploadService implements UploadService {
  ApiUploadService(this._api, {http.Client? httpClient}) : _http = httpClient ?? http.Client();
  ...
  Future<String> upload({
    ...
    final signed = await _api.post('/uploads/sign', body: {
      'kind': kind.wire,
  ...
  @override
  Future<({Uint8List bytes, String contentType})> readRecording(String url) async {
    final res = await _http.get(Uri.parse(url));
    final contentType = (res.headers['content-type'] ?? 'audio/webm').split(';').first.trim();
