...
void main() {
  ...
  test('signs with kind + contentType, PUTs the bytes, and returns the path', () async {
    ...
    final service = ApiUploadService(api, httpClient: client);
    ...
    final body = verify(() => api.post('/uploads/sign', body: captureAny(named: 'body')))
        .captured
        .single as Map;
  ...
  test('sends audio kind for an audio note', () async {
    when(() => api.post('/uploads/sign', body: any(named: 'body'))).thenAnswer(
      (_) async => {'uploadUrl': 'https://storage.example/u', 'path': 'material-requests/sup1/u.m4a'},
    );
    final client = MockClient((_) async => http.Response('', 200));
    final service = ApiUploadService(api, httpClient: client);

    await service.upload(kind: AttachmentKind.audio, bytes: bytes, contentType: 'audio/mp4');
  ...
  test('throws ApiException when the PUT fails', () async {
    when(() => api.post('/uploads/sign', body: any(named: 'body'))).thenAnswer(
      (_) async => {'uploadUrl': 'https://storage.example/u', 'path': 'material-requests/sup1/u.jpg'},
    );
    final client = MockClient((_) async => http.Response('denied', 403));
    final service = ApiUploadService(api, httpClient: client);

    expect(
      () => service.upload(kind: AttachmentKind.photo, bytes: bytes, contentType: 'image/jpeg'),
      throwsA(isA<ApiException>().having((e) => e.statusCode, 'statusCode', 403)),
    );
  });

  test('readRecording fetches the blob bytes + normalized content type', () async {
  ...
  group('photoContentType', () {
    ...
    test('sniffs PNG / JPEG / WebP magic bytes', () {
      ...
      // RIFF....WEBP
      expect(
        photoContentType(bytesOf([0x52, 0x49, 0x46, 0x46, 0, 0, 0, 0, 0x57, 0x45, 0x42, 0x50])),
        'image/webp',
      );
    });
    ...
    test('detects HEIC via the ISO-BMFF ftyp box', () {
      // bytes 4..7 == 'ftyp'
      expect(photoContentType(bytesOf([0, 0, 0, 0, 0x66, 0x74, 0x79, 0x70])), 'image/heic');
    });
    ...
    test('beats a wrong reported mime type — bytes win (web re-encode case)', () {
      ...
      expect(
        photoContentType(bytesOf([0x89, 0x50, 0x4E, 0x47]), mimeType: 'image/heic', fileName: 'x.heic'),
        'image/png',
      );
    });

    test('falls back to a reported allowed mime when bytes are inconclusive', () {
      expect(photoContentType(Uint8List(4), mimeType: 'image/webp'), 'image/webp');
    });

    test('falls back to the file extension, then defaults to jpeg', () {
      expect(photoContentType(Uint8List(4), fileName: 'photo.PNG'), 'image/png');
      expect(photoContentType(Uint8List(4), fileName: 'photo.heif'), 'image/heic');
